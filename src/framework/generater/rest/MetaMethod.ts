...
export class MetaMethod {
  ...
  toClientDefString(): string {
    ...
    let str = "";
    str += `  async ${this.name}(${this.params
      .map((p) => p.toClientDefString())
  ...
  toExpressRoutingString(className: string): string {
    ...
    const paramObjectName =
      this.httpMethodNoQuote === "get" ? "req.query" : "req.body";
    let count = 0;
    const paramString = this.params
      .map((p) => p.toRestString(paramObjectName, count++))
      .join(",");
    let str = "";
