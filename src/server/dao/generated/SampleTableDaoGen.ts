...
export class SampleTableDaoGen extends BaseDao {
  ...
  async selectAll(): Promise<SampleTable[]> {
    ...
    return await this.db.query<SampleTable>(this.con, sql);
  }
  // SampleTableを主キーで取得
  async selectById(id: number): Promise<SampleTable> {
    const sql = 'SELECT * FROM sample_table WHERE  id = ?  ';
    const params = [id];
    const results = await this.db.query<SampleTable>(this.con, sql, params);
    return results[0];
  }
  // SampleTableを主キーで削除
  async deleteById(id: number): Promise<ResultSetHeader> {
    ...
    const params = [id];
    return await this.db.execute(this.con, sql, params);
  }
  // SampleTableを挿入
